    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-neu-black/70"
      ...
      <div className="w-full max-w-md bg-neu-white border-2 border-neu-black shadow-neu-xl">
        ...
        <div className="px-5 py-4 flex items-center justify-between">
          ...
          <button onClick={handleClose} className={cn('px-4 py-2 bg-neu-white border-2 border-neu-black shadow-neu font-display font-bold text-xs uppercase tracking-wide text-neu-black hover:shadow-neu-sm')}>
            {t('common.close')}
          </button>
        </div>
      </div>
  ...
  return (
    <tr className="border-b-2 border-neu-black bg-neu-white hover:bg-neu-bg">
      ...
      <td className="px-4 py-3 w-52">
        <div className="flex items-center gap-2">
          <button onClick={() => onToggleActive(account)} className={cn('px-2.5 py-1 border-2 border-neu-black font-display font-bold text-[10px] uppercase tracking-wide shadow-neu-sm hover:shadow-none', account.isActive ? 'bg-neu-black/10 text-neu-black' : 'bg-neu-green text-neu-white')}>
            {account.isActive ? t('common.deactivate') : t('common.activate')}
          </button>
          <button onClick={() => onEdit(account.id)} className="px-2.5 py-1 border-2 border-neu-black bg-neu-primary font-display font-bold text-[10px] uppercase tracking-wide text-neu-black shadow-neu-sm hover:shadow-none">{t('common.edit')}</button>
          <button onClick={() => onDelete(account)} className="px-2.5 py-1 border-2 border-neu-black bg-neu-white font-display font-bold text-[10px] uppercase tracking-wide text-neu-accent shadow-neu-sm hover:bg-neu-accent hover:text-neu-white hover:shadow-none">{t('common.delete')}</button>
        </div>
      </td>
    </tr>
  );
