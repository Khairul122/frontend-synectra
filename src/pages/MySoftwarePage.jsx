...
const STATUS_BG = {
  ...
  pending_verification: { bg: 'bg-neu-blue',     text: 'text-neu-white' },
...
export default function MySoftwarePage() {
  ...
  useEffect(() => {
    const init = async () => {
      try { const me = await authService.getMe(); setUser(me.data); await loadData(); }
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      await loadData();
      alert.success(t('client.mySoftware.buySuccess', { name: buyTarget.name }));
      setBuyTarget(null);
    } catch (err) {
      const msg = err?.response?.data?.message ?? t('client.mySoftware.buyFailed');
  ...
  return (
    <PageLayout user={user} title={t('client.mySoftware.title')} alert={alert}>
      <ConfirmModal
        ...
        message={t('client.mySoftware.confirmMsg', { name: buyTarget?.name, price: fmt(buyTarget?.price ?? 0) })}
        onConfirm={handleBuy}
      ...
      {uploadTarget && (
        <UploadReceiptModal
          ...
          onUploaded={async () => { await loadData(); alert.success(t('client.uploadReceipt.send')); }}
        />
      )}
      ...
      <div ref={pageRef} className="space-y-10">
        ...
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="font-display font-bold text-xl uppercase tracking-wide">{t('client.mySoftware.catalog')}</h2>
          </div>
          {software.length === 0 ? (
            <div className="border-2 border-dashed border-neu-black p-12 text-center">
              <p className="font-body text-neu-black/40">{t('client.mySoftware.noCatalog')}</p>
            </div>
          ) : (
        ...
        <section>
          <div className="flex items-center gap-3 mb-6">
            <div className="h-1 w-6 bg-neu-black" />
            <h2 className="font-display font-bold text-xl uppercase tracking-wide">{t('client.mySoftware.history')}</h2>
          </div>
          {purchases.length === 0 ? (
            <div className="border-2 border-dashed border-neu-black p-12 text-center">
              <p className="font-body text-neu-black/40">{t('client.mySoftware.noHistory')}</p>
            </div>
          ) : (
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="bg-neu-black text-neu-white">
                      {['No', t('client.mySoftware.cols.software'), t('client.mySoftware.cols.price'), t('common.status'), t('client.mySoftware.cols.date'), t('common.actions')].map((h, i) => (
                        <th key={h} className={cn('px-4 py-3 font-display font-bold text-xs uppercase tracking-wide text-left', i < 5 && 'border-r-2 border-neu-white/20')}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {purchases.map((p, idx) => {
                      ...
                      return (
                        <tr key={p.id} className="border-b-2 border-neu-black bg-neu-white hover:bg-neu-bg transition-colors">
                          ...
                          <td className="px-4 py-3 border-r-2 border-neu-black font-display font-bold text-sm text-neu-black whitespace-nowrap">{fmt(p.totalPrice)}</td>
                          <td className="px-4 py-3 border-r-2 border-neu-black">
                            <span className={cn('inline-block px-2 py-0.5 border-2 border-neu-black font-mono font-bold text-xs uppercase', cfg.bg, cfg.text)}>
                              {t(`status.${p.paymentStatus}`, { defaultValue: p.paymentStatus })}
                            </span>
                            {p.paymentStatus === 'rejected' && p.notes && (
                              <p className="font-body text-xs text-neu-accent mt-1">{t('client.mySoftware.adminNote')} {p.notes}</p>
                            )}
                          </td>
