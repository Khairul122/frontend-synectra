...
export function ConfirmModal({ isOpen, title, message, onConfirm, onCancel, isLoading }) {
  ...
  return createPortal(
    ...
    >
      <div
        ref={cardRef}
        className="w-full max-w-sm bg-neu-white border-2 border-neu-black shadow-neu-xl"
      >
        {/* Header */}
        ...
        <div className="flex gap-3 px-5 pb-5">
          ...
          <button
            ...
            className={cn(
              'flex-1 py-2.5 font-display font-bold text-xs uppercase tracking-wide',
              'bg-neu-accent text-neu-white border-2 border-neu-black shadow-neu',
              'transition-all duration-150',
              'hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-neu-sm',
              'active:translate-x-1 active:translate-y-1 active:shadow-none',
          ...
          >
            {isLoading ? (
              <span className="inline-flex items-center gap-1 justify-center">
                <span className="animate-spin">⟳</span> Keluar...
              </span>
            ) : 'Ya, Keluar'}
          </button>
        </div>
