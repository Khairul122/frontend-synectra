  ...
  return (
    <section id="software" className="border-b-2 border-neu-black bg-neu-black py-16 lg:py-20">
      <div className="max-w-7xl mx-auto px-4 lg:px-6">
          ...
          {!error && (
            <div className="flex gap-2">
              ...
              <button
                ...
                className="w-10 h-10 border-2 border-neu-gold rounded-neu bg-neu-gold shadow-neu-solid-sm flex items-center justify-center font-bold hover:bg-neu-white active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all"
              >
                →
              </button>
            </div>
        ...
        {error ? (
          <ErrorState message="Gagal memuat daftar software." dark />
        ) : (
          ...
          {softwareProducts.map(sw => {
            ...
            return (
              <TiltCard key={sw.id} maxTilt={5} className="flex-shrink-0 w-72">
                <motion.div onClick={() => setActiveSoftware(sw)} {...cardAnim(0)} className="flex flex-col bg-neu-white border-2 border-neu-black rounded-neu-lg shadow-neu-solid transition-shadow duration-200 hover:shadow-neu-solid-lg cursor-pointer select-none">

                  {/* Thumbnail / Flat Graphic */}
                  <div className="relative border-b-2 border-neu-black h-40 bg-neu-cream overflow-hidden flex items-center justify-center">
                    {sw.thumbnailUrl ? (
                    ...
                    <div className="flex gap-1.5">
                      {sw.demoUrl && (
                        <a href={sw.demoUrl} target="_blank" rel="noopener noreferrer"
                          ...
                          className="px-2.5 py-1.5 border-2 border-neu-black rounded-neu-sm bg-neu-white font-display font-bold text-[10px] uppercase text-neu-black transition-all duration-150 hover:bg-neu-cream hover:translate-x-[1px] hover:translate-y-[1px]">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-neu-gold border-2 border-neu-black rounded-neu-sm shadow-neu-solid-sm font-display font-bold text-[10px] uppercase text-neu-black transition-all duration-150 hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none">
