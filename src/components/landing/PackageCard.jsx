...
export function PackageCard({ pkg, onOrder }) {
  ...
  return (
    <div className="relative border-2 border-neu-black rounded-neu-lg shadow-neu bg-neu-white flex flex-col h-full overflow-hidden transition-all duration-300 hover:-translate-x-1.5 hover:-translate-y-1.5 hover:shadow-neu-lg hover:rotate-[-0.5deg]">
      {/* Badge */}
      {pkg.badge && (
        <span className="absolute -top-3 left-4 px-3 py-0.5 bg-neu-primary border-2 border-neu-black rounded-neu-sm font-mono font-bold text-[10px] uppercase z-10">
          {pkg.badge}
        </span>
      )}
          ...
          <ul className="space-y-2">
            {featureList.map((f, i) => (
              <li key={i} className="flex items-start gap-2">
                <div className="w-4 h-4 border-2 border-neu-black rounded-neu-sm bg-neu-primary flex items-center justify-center flex-shrink-0 mt-0.5">
      ...
      <div className="p-5 border-t-2 border-neu-black">
        <button
          onClick={onOrder}
          className="w-full py-2.5 bg-neu-primary border-2 border-neu-black rounded-neu-sm shadow-neu font-display font-bold text-sm uppercase text-neu-black transition-all duration-150 hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-neu-sm active:translate-x-[4px] active:translate-y-[4px] active:shadow-none"
        >
          {t('landing.order')}
        </button>
      </div>
