...
export function WhyChooseUs() {
  ...
  return (
    <section className="border-b-2 border-t-4 border-neu-black bg-paper-grid overflow-hidden">
      {/* Static value-prop band */}
      <div className="bg-neu-gold border-b-2 border-neu-black py-3 px-4 lg:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0 border-l-2 border-t-2 border-neu-black rounded-neu overflow-hidden">
          {items.map((w, i) => (
            <motion.div
              ...
              className="border-r-2 border-b-2 border-neu-black p-6 lg:p-8 hover:bg-neu-white transition-colors duration-150"
            >
              <div className="flex items-center gap-2 mb-4">
                <span className="font-mono font-bold text-xs text-neu-gold bg-neu-black rounded-neu-sm px-1.5 py-0.5">{String(i + 1).padStart(2, '0')}</span>
              </div>
