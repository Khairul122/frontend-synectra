  ...
  return (
    <>
      <motion.div
        ...
        style={{ scaleX }}
      />
      <nav className="sticky top-0 z-40 bg-neu-cream border-b-2 border-neu-black">
        <div className="max-w-7xl mx-auto px-4 lg:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-5">
            <picture>
              ...
              <img src="/logo-synectra.jpeg" alt="Synectra" width="120" height="36"
                className="h-9 w-auto max-w-[120px] border-2 border-neu-black rounded-neu object-contain shadow-neu-solid-sm" />
            </picture>
              ...
              {NAV_LINKS.map(({ tKey, id }) => (
                <button key={id}
                  ...
                  className={cn(
                    ...
                    activeSection === id
                      ? 'text-neu-black border-b-2 border-neu-gold'
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            <button onClick={() => transitionTo('/login')} className="px-4 py-2 border-2 border-neu-black rounded-neu font-display font-bold text-xs uppercase text-neu-black hover:bg-neu-white transition-colors">{t('nav.login')}</button>
          ...
          <button
            ...
            className="sm:hidden w-11 h-11 border-2 border-neu-black rounded-neu flex items-center justify-center">
        ...
        {menuOpen && (
          <div className="sm:hidden border-t-2 border-neu-black bg-neu-cream px-4 py-3 flex flex-col gap-1">
            {NAV_LINKS.map(({ tKey, id }) => (
              <button key={id}
            ...
            <div className="pt-2 flex flex-col gap-2">
              ...
              <button onClick={() => { transitionTo('/register'); setMenuOpen(false); }} className="px-4 py-2 bg-neu-gold border-2 border-neu-black rounded-neu font-display font-bold text-sm uppercase text-neu-black text-center">{t('nav.register')}</button>
