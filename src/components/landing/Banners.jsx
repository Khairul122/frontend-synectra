...
export function Banners({ banners, setBannerModal, setBannerModalExp }) {
  ...
  return (
    <section className="border-b-2 border-neu-black bg-neu-gold py-10">
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 snap-x -mx-4 px-4 lg:mx-0 lg:px-0">
          {banners.map((b, bi) => (
            <motion.div key={b.id} {...cardAnim(Math.min(bi * STAGGER, 0.42))}
              ...
              className="flex-shrink-0 w-72 snap-start border-2 border-neu-black rounded-neu-lg shadow-neu-solid bg-neu-white overflow-hidden cursor-pointer group hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-neu-solid-lg transition-all duration-150">
              {b.image && (
